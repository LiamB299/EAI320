...
def BackPropagation(dn, cn_target, dhw, hcw, learning_rate, epoch, error):
    loss = []
    e = 0
    cn_comp = []
    for iteration in range(epoch):
        ...
        # hidden to output
        cn_comp, in3 = forward_layer_propagation(hn1, hcw)
        ...
        hcw = weight_update(hcw, j_del, learning_rate, hn1)
        ...
        dhw = weight_update(dhw, i_del_1, learning_rate, dn)

    return dhw, hcw, loss, cn_comp
...
def tests(N_samples, sets, iterate, error_stop, learn_rate, hidden_nodes):
    ...
    DHW, HCW, loss, CN = BackPropagation(dn, cn, Win, Wout, learn_rate, iterate, error_stop)
...
# ======================================
    ...
    for i in range(1, sets):
        ...
        DHW, HCW, loss, CN = BackPropagation(dn, cn, DHW, HCW, learn_rate, iterate, error_stop)
        loss_total.extend(loss)
        wrong, correct = real_test(cn, CN)
        wins.append(correct)
        losses.append(wrong)
