...
def BackPropagation(dn, cn_target, dhw, hhw_1, hcw, learning_rate, epoch, error, hhw_2):
    loss = []
    e = 0
    cn_comp = []
    for iteration in range(epoch):
        ...
        # hidden to output
        cn_comp, in4 = forward_layer_propagation(hn3, hcw)
        ...
        j_del = output_back_gradient(in4, E)
        # ======================================

        # hidden 3 to hidden 2 gradient calculation
        i_del_1 = back_gradient(in4, hcw, j_del)

        # hidden 2 to hidden 1 gradient calculation
        i_del_2 = back_gradient(in3, hhw_2, i_del_1)

        # hidden 1 to input
        i_del_3 = back_gradient(in2, hhw_1, i_del_2)

        # =======================================

        # update output to hidden weights
        hcw = weight_update(hcw, j_del, learning_rate, hn3)
        ...
        hhw_2 = weight_update(hhw_2, i_del_1, learning_rate, hn2)

        # remove bias before adjusting weights of previous layer
        i_del_2 = np.delete(i_del_2, np.size(i_del_2) - 1, axis=0)

        # update hidden 2 to hidden 1 weights
        hhw_1 = weight_update(hhw_1, i_del_2, learning_rate, hn1)

        # remove bias before adjusting weights of previous layer
        i_del_3 = np.delete(i_del_3, np.size(i_del_3) - 1, axis=0)

        # update hidden in input layer weights
        dhw = weight_update(dhw, i_del_3, learning_rate, dn)

    return dhw, hhw_1, hhw_2, hcw, loss, cn_comp
...
def tests(N_samples, sets, iterate, error_stop, learn_rate, hidden_nodes):
    ...
    DHW, HHW_1, HHW_2, HCW, loss, CN = BackPropagation(dn, cn, Win, Whidden_2, Wout, learn_rate, iterate, error_stop, Whidden_3)
...
# ======================================
    ...
    for i in range(1, sets):
        ...
        DHW, HHW_1, HHW_2, HCW, loss, CN = BackPropagation(dn, cn, DHW, HHW_1, HCW, learn_rate, iterate, error_stop, HHW_2)
        loss_total.extend(loss)
        wrong, correct = real_test(cn, CN)
        wins.append(correct)
        losses.append(wrong)
