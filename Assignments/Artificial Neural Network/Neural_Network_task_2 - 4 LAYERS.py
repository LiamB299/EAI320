...
def BackPropagation(dn, cn_target, dhw, hhw_1, hcw, learning_rate, epoch, error, hhw_2, hhw_3):
    loss = []
    e = 0
    cn_comp = []
    for iteration in range(epoch):
        ...
        # hidden to output
        cn_comp, in5 = forward_layer_propagation(hn4, hcw)
        ...
        # output to hidden 4 gradient calculation
        j_del = output_back_gradient(in5, E)
        # ======================================

        # hidden 4 to hidden 3 gradient calculation
        i_del_1 = back_gradient(in5, hcw, j_del)

        # hidden 3 to hidden 2 gradient calculation
        i_del_2 = back_gradient(in4, hhw_3, i_del_1)

        # hidden 2 to hidden 1 gradient calculation
        i_del_3 = back_gradient(in3, hhw_2, i_del_2)

        # hidden 1 to input
        i_del_4 = back_gradient(in2, hhw_1, i_del_3)

        # =======================================

        # update output to hidden weights
        hcw = weight_update(hcw, j_del, learning_rate, hn4)
...
def tests(N_samples, sets, iterate, error_stop, learn_rate, hidden_nodes):
    ...
    DHW, HHW_1, HHW_2, HHW_3, HCW, loss, CN = BackPropagation(dn, cn, Win, Whidden_2, Wout, learn_rate, iterate, error_stop, Whidden_3, Whidden_4)
...
# ======================================
    ...
    for i in range(1, sets):
        ...
        DHW, HHW_1, HHW_2, HHW_3, HCW, loss, CN = BackPropagation(dn, cn, DHW, HHW_1, HCW, learn_rate, iterate, error_stop, HHW_2, HHW_3)
        loss_total.extend(loss)
        wrong, correct = real_test(cn, CN)
        wins.append(correct)
        losses.append(wrong)
