...
def BackPropagation(dn, cn_target, dhw, hhw_1, hcw, learning_rate, epoch, error):
    loss = []
    e = 0
    cn_comp = []
    for iteration in range(epoch):
        ...
        if len(loss) > 1:
            if convergence_criterion(loss[len(loss) - 2], loss[len(loss) - 1], error):
                break
...
def tests(N_samples, sets, iterate, error_stop, learn_rate, hidden_nodes):
    ...
    # ==============================================
    DHW, HHW_1, HCW, loss, CN = BackPropagation(dn, cn, Win, Whidden_2, Wout, learn_rate, iterate, error_stop)
    ...
    for i in range(1, sets):
        ...
        DHW, HHW_1, HCW, loss, CN = BackPropagation(dn, cn, DHW, HHW_1, HCW, learn_rate, iterate, error_stop)
        loss_total.extend(loss)
        wrong, correct = real_test(cn, CN)
        wins.append(correct)
        losses.append(wrong)
